#!/usr/bin/env node
/* token-meter-exclude-thinking — 让 dsh-token-meter 的 totalTokens 只算可见上下文
 * （输入 + 回答 + 工具结果），把 transient 且（home 下）不回放的历史思考剔除出压缩触发阈值。
 * 背景：pi-ai 把 reasoning 折进 outputTokens，导致 "27.3K 可见上下文" 也会被
 * compaction-basic 以 0.8×contextWindow 提前压缩。补丁共 5 处，全部带
 * `exclude-thinking-from-meter` 标记。幂等；--dry-run 可预览。
...
const edits = [
  // 1. estimateContent: reasoning 块不再计价（可见上下文只算 text/tool/…）
  ['\t\tcase "text":\n\t\tcase "reasoning":\n\t\t\ttokens += Math.ceil(block.text.length / CHARS_PER_TOKEN) + BLOCK_OVERHEAD;\n\t\t\tbreak;',
   '\t\tcase "text":\n\t\t\ttokens += Math.ceil(block.text.length / CHARS_PER_TOKEN) + BLOCK_OVERHEAD;\n\t\t\tbreak;\n\t\tcase "reasoning":\n\t\t\t// LOCAL PATCH (exclude-thinking-from-meter): reasoning is transient and, for\n\t\t\t// home/Qwen (reasoning replay disabled via qreasonIds), never replayed - it must\n\t\t\t// not count toward context pressure. See repo README.\n\t\t\tbreak;'],
  // 2. estimateReasoning helper（只累加 reasoning 块，供 usage 路径减法用）
  ...
  // 3a. _estimateProviderAssistant fallback -> object
  ['\t\tif (sourceSeqs === void 0) return durableEventTokens;',
   '\t\tif (sourceSeqs === void 0) return { tokens: durableEventTokens, reasoningTokens: 0 };'],
  // 3b. _estimateProviderAssistant return -> object
  ...
  // 5. measure() usage 基线减去当前轮 reasoning
  ['\t\t\tbaseline = usage !== void 0 && usageTokens(usage) >= estimatedAnchorTokens ? {\n\t\t\t\tkind: "usage",\n\t\t\t\ttokens: usageTokens(usage),\n\t\t\t\tusage\n\t\t\t} : {',
   '\t\t\tbaseline = usage !== void 0 && usageTokens(usage) >= estimatedAnchorTokens ? {\n\t\t\t\tkind: "usage",\n\t\t\t\ttokens: Math.max(0, usageTokens(usage) - (anchor.reasoningTokens ?? 0)),\n\t\t\t\tusage\n\t\t\t} : {'],
];
...
for (let i = 0; i < edits.length; i++) {
  ...
  if (n !== 1) { console.error('edit #' + (i + 1) + ' MISMATCH (' + n + ' occurrences) — token-meter dist shape changed; patch manually per repo README.'); fail++; continue; }
  body = body.replace(oldStr, newStr);
  console.log('edit #' + (i + 1) + ' OK');
}
if (fail) { console.error('aborted: ' + fail + ' edit(s) mismatched, no write.'); process.exit(1); }
